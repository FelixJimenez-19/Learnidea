...
CREATE TABLE seccion_leccion (
    ...
    curso_seccion_id INT,
    UNIQUE(seccion_leccion_descripcion(255), curso_seccion_id),
    FOREIGN KEY (curso_seccion_id) REFERENCES curso_seccion (curso_seccion_id) ON DELETE CASCADE
) ENGINE INNODB;
...
CREATE TABLE seccion_pregunta (
    ...
    UNIQUE(seccion_pregunta_descripcion(255), seccion_leccion_id),
...
CREATE TABLE transaccion_tipo (
    ...
    transaccion_tipo_credito BOOLEAN,       -- //Solo una opcion a la vez
    transaccion_tipo_entrada BOOLEAN,        -- //Solo una opcion a la vez
    transaccion_tipo_salida BOOLEAN          -- //Solo una opcion a la vez
) ENGINE INNODB;
